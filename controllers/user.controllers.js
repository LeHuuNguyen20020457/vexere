...
const login = async (req, res) => {
    try {
        ...
        if (user) {
            ...
            console.log(isAuth);
            if (isAuth) {
                const token = jwt.sign({ email: user.email, type: user.type }, process.env.ACCESS_TOKEN_SECRET, {
                    expiresIn: '90000s',
                });
...
const getAllTrip = async (req, res) => {
    try {
        const TripsOfUser = await sequelize.query(
            `SELECT users.name as username, stations.name as fromStatation, S.name as toStatation
    FROM ((users 
    ...
    INNER JOIN stations AS S ON S.id = trips.toStationID)`,
